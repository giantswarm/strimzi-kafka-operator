...
const (
	...
	// kafkaPoolName is the KafkaNodePool name; the StatefulSet will be <cluster>-<pool>.
...
func TestWC(t *testing.T) {
	suite.New().
		...
		AfterClusterReady(func() {
			By("verifying WC API connectivity")
			...
			Describe("strimzi-kafka-operator", Ordered, func() {
				...
				Describe("kafka integration", Ordered, func() {
					...
					BeforeAll(func() {
						...
						Expect(wcClient.Create(ctx, kafkaNodePoolRef)).To(Succeed())

						By("deploying Kafka CR")
						kafkaRef = kafkaManifest()
						Expect(wcClient.Create(ctx, kafkaRef)).To(Succeed())
					...
					It("Kafka broker StatefulSet should become ready", func() {
						ctx := state.GetContext()
						stsName := fmt.Sprintf("%s-%s", kafkaClusterName, kafkaPoolName)
						Eventually(func() (bool, error) {
							...
							"Kafka broker StatefulSet %s/%s never became ready", kafkaNamespace, stsName)
					})
					...
					It("Kafka broker metrics should appear in Mimir", func() {
						...
						promQL := fmt.Sprintf(`kafka_server_broker_state{namespace=%q}`, kafkaNamespace)
