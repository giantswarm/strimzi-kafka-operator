...
| crds.install | bool | `true` | Whether to install/upgrade CRDs from templates/crds/. Set to false to manage CRDs externally. |
...
| verticalPodAutoscaler.enabled | bool | `true` | Create a VerticalPodAutoscaler for the operator. Requires VPA CRDs on the cluster. |
| verticalPodAutoscaler.updateMode | string | `"Recreate"` | VPA update mode: Recreate restarts pods to apply recommendations; Initial applies only at creation; Off disables updates. |
...
| podMonitor.enabled | bool | `true` | Create a PodMonitor to scrape operator metrics on port 8080. |
| podMonitor.labels | object | `{"observability.giantswarm.io/tenant":"giantswarm"}` | Labels added to the PodMonitor resource. Use 'observability.giantswarm.io/tenant' to route metrics to a Grafana Organization. |
| podMonitor.workloads | object | `{"enabled":true}` | PodMonitors for Kafka workloads managed by the operator (cross-namespace). |
| podMonitor.workloads.enabled | bool | `true` | Create PodMonitors for Kafka, KafkaBridge, and Entity Operator pods across all namespaces. |
...
| dashboards.enabled | bool | `true` | Create Grafana dashboard ConfigMaps from files/grafana-dashboards/. Run `make sync-dashboards` after a version bump to refresh. |
| dashboards.namespace | string | `""` | Namespace for dashboard ConfigMaps. Defaults to .Release.Namespace when empty. |
| dashboards.label | string | `"app.giantswarm.io/kind"` | ConfigMap label key used for Grafana dashboard discovery. |
| dashboards.labelValue | string | `"dashboard"` | ConfigMap label value used for Grafana dashboard discovery. |
| dashboards.annotations | object | `{"observability.giantswarm.io/folder":"Kafka","observability.giantswarm.io/organization":"Shared Org"}` | Annotations added to each dashboard ConfigMap (e.g. observability.giantswarm.io/organization). |
...
| strimzi-kafka-operator.enabled | bool | `true` | Enable the upstream strimzi-kafka-operator subchart. |
...
| strimzi-kafka-operator.extraEnvs | list | `[{"name":"STRIMZI_POD_SECURITY_PROVIDER_CLASS","value":"io.strimzi.plugin.security.profiles.impl.RestrictedPodSecurityProvider"}]` | Additional environment variables to inject into the operator container. |
